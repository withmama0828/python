...
class largeXMLDealer:
    ...
    def __call__(self, *args, **kwargs):
        if len(args) == 2:
            print("解析largeXMLParse " + args[0] + " " + args[1])
    ...
    def parse(self, fileName, elemTag, func4Element=None):
        ...
        for event, elem in context:
            ...
            try:
                ...
                raise Exception("参数出错: func4Element")
            finally:
                elem.clear()
                count = count + 1
        ...
        del context
        # Return how many elements had been parsed# 返回已解析的元素数
        return count
...
def XMLParsing(fileName, elemTag):
    print("解析 %s xml文件的 %s 元素完成" % (fileName,elemTag))


if __name__ == "__main__":
    print("输入XML文件名和解析元素名")
    while True:
        fileName = input("filename：")
        elemTag = input("elemtag：")
        # print(fileName)
        # print(elemTag)
